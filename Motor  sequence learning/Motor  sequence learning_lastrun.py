...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Ishika\\OneDrive\\Desktop\\college\\Lab in psychology\\Motor  sequence learning\\Motor  sequence learning_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixation2 = visual.ShapeStim(
        ...
        opacity=None, depth=0.0, interpolate=True)
    line11 = visual.Rect(
        win=win, name='line11',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(-150, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-1.0, interpolate=True)
    line22 = visual.Rect(
        win=win, name='line22',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(-50, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-2.0, interpolate=True)
    line33 = visual.Rect(
        win=win, name='line33',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(50, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-3.0, interpolate=True)
    line44 = visual.Rect(
        win=win, name='line44',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(150, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    probe2 = visual.ShapeStim(
        win=win, name='probe2',units='pix', 
        size=(10, 10), vertices='triangle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-5.0, interpolate=True)
    ...
    for thisTrial in trials:
        ...
        trialComponents = [fixation, line1, line_2, line_3, line_4, probe, key_resp]
    ...
    for thisTrial_2 in trials_2:
        ...
        probe2.setPos((pos, 60))
        # create starting attributes for key_resp_2
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        random_2Components = [fixation2, line11, line22, line33, line44, probe2, key_resp_2]
        ...
        while continueRoutine:
            ...
            # if line11 is starting this frame...
            if line11.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line11.frameNStart = frameN  # exact frame index
                line11.tStart = t  # local t and not account for scr refresh
                line11.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line11, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line11.started')
                # update status
                line11.status = STARTED
                line11.setAutoDraw(True)
            
            # if line11 is active this frame...
            if line11.status == STARTED:
                # update params
                pass
            
            # *line22* updates
            
            # if line22 is starting this frame...
            if line22.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line22.frameNStart = frameN  # exact frame index
                line22.tStart = t  # local t and not account for scr refresh
                line22.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line22, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line22.started')
                # update status
                line22.status = STARTED
                line22.setAutoDraw(True)
            
            # if line22 is active this frame...
            if line22.status == STARTED:
                # update params
                pass
            
            # *line33* updates
            
            # if line33 is starting this frame...
            if line33.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line33.frameNStart = frameN  # exact frame index
                line33.tStart = t  # local t and not account for scr refresh
                line33.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line33, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line33.started')
                # update status
                line33.status = STARTED
                line33.setAutoDraw(True)
            
            # if line33 is active this frame...
            if line33.status == STARTED:
                # update params
                pass
            
            # *line44* updates
            
            # if line44 is starting this frame...
            if line44.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line44.frameNStart = frameN  # exact frame index
                line44.tStart = t  # local t and not account for scr refresh
                line44.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line44, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line44.started')
                # update status
                line44.status = STARTED
                line44.setAutoDraw(True)
            
            # if line44 is active this frame...
            if line44.status == STARTED:
                # update params
                pass
            ...
            if probe2.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
